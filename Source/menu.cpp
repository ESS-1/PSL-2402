...
void TMenuItem::EditEnter(void)
{
  Edit = 1;
  Par->ShowValue();
  if(Par->Type == PT_V || Par->Type == PT_PRV)
...
void TMenuMain::Execute(void)
{
  ...
  {
    uint32_t p = (uint32_t)Analog->AdcV->Value * Analog->AdcI->Value / 1000;
    Analog->AdcV->Sync();
    Analog->AdcI->Sync();
    if(Edit && ParIndex == PAR_V)
    {
      Display->SetPos(1, 0);
      Display->PutIntF(p, 3, 2 + AUTO_SCALE);
    }
  ...
  if(!Power && (Analog->AdcV->Ready() || ForceV) && !(Edit && ParIndex == PAR_V))
  {
       ...
       (Analog->IsCV())))
    {
      ...
      Display->SetPos(0, 0);
      Display->PutIntF(Analog->AdcV->Value, 3, 2 + AUTO_SCALE);
    }
    else
    {
  ...
  if(!Power && Analog->AdcI->FastUpdate && !(Edit && ParIndex == PAR_I))
  {
    ...
    if(Dnp)
    {
      ...
      else
      {
        ...
        {
          Display->SetPos(1, 0);
          Display->PutIntF(0, 3, 0 + AUTO_RANGE);
        }
      }
    }
    ...
    else if(Analog->AdcI->Ready() || ForceI)
    {
         ...
         (Analog->IsCC())))
      {
        //то отображается измеренное значение:
        Display->SetPos(1, 0);
        Display->PutIntF(Analog->AdcI->Value, 3, 0 + AUTO_RANGE);
      }
      else
      {
...
void TMenuMain::OnKeyboard(KeyMsg_t &msg)
{
  ...
  if(msg == KBD_ENC)
  {
    ...
    {
      EditExit();
      //вход в редактирование I, если V не регулировалось:
  ...
  //кнопка OUT ON/OFF - выход из редактирования:
  if(msg == KBD_OUT)
  {
    EditExit();
...
void TMenuTop::OnKeyboard(KeyMsg_t &msg)
{
  ...
  //удержание кнопки - выход в MNU_MAIN:
  if(msg & KBD_HOLD)
  {
    EditEscape();
    MnuIndex = MNU_MAIN;
