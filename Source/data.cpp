...
void TParam::ShowValue(void)
{
  ...
  {
  case PT_V:
  case PT_PRV:   Display->PutIntF(Value, 3, 2 + AUTO_SCALE);
                 break;
  case PT_I:
  case PT_PRI:   Display->PutIntF(Value, 3, 0 + AUTO_RANGE);
                 break;
  case PT_P:
  case PT_PRP:   Display->PutIntF(Value, 3, 1);
                 break;
  case PT_PRE:   if(Value == 0) Display->PutString("GEt");
                 if(Value == 1) Display->PutString("SEt");
                 break;
  ...
  case PT_APHPL: if(Value == 0) Display->PutString(" AG");
                 ...
                 break;
  case PT_DEL:   Display->PutIntF(Value, 3, 0);
                 break;
  case PT_T:     Display->PutIntF(Value / 10, 2, 0);
                 Display->PutChar('*');
                 break;
  case PT_FIRM:  Display->PutIntF(Value, 3, 2);
                 break;
...
bool TParam::Edit(int16_t step)
{
  ...
  Validate();
  //адаптивный шаг для PT_V, PT_I, PT_PRV, PT_PRI,
  //шаг x10 для PT_T:
  if(((Type == PT_V || Type == PT_I || Type == PT_PRV || Type == PT_PRI) &&
      Value > 999) || Type == PT_T)
    step = step * 10;
  //выравнивание на шаг:
...
inline void TData::InitPresets(void)
{
  static const uint16_t PRE_V_INIT[PRESETS] =
    {330, 500, 900, 1000, 1200, 1500, 1800, 2000, 2200, 2400 };

  static const uint16_t PRE_I_INIT = 500;
